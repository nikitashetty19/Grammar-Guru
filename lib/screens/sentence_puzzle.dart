...
class _SentencePuzzleState extends State<SentencePuzzle> {
  ...
  List<String> sentences = ["Hi, What are you! doing today?", "", "", "", ""];
  ...
  List<Widget> buildSentence(String sentence) {
    ...
    sentence.split(' ').forEach((word) {
      List<Widget> wordWidgets = word.split('').map((char) {
        if (draggablePunctuations.contains(char)) {
          // Wrap your punctuation drag target with a little bit of padding if needed
          return Padding(
            padding: EdgeInsets.symmetric(
                horizontal: 1), // Adjust the padding if necessary
            child: buildPunctuationDragTarget(char),
          );
        } else {
          // Wrap each character with a little bit of padding
          return Padding(
            padding: EdgeInsets.symmetric(
                horizontal: 1), // Adjust the padding if necessary
            child: Text(
              ...
              style: TextStyle(
                ...
                color: Colors.black,
                fontFamily:
                    'RobotoMono', // Use a monospaced font like Roboto Mono or any other of your choice
              ),
            ),
          );
        }
      ...
      // Add the whole word to the sentence parts
      sentenceParts.add(Wrap(children: wordWidgets));
    });

    // Return the sentence as a Wrap widget with all parts
    return [
      Wrap(
        spacing: 4, // Adjust the space between words
        runSpacing: 4, // Adjust the space between lines
  ...
  Future<void> sendDataToBackend() async {
    try {
      var url = Uri.parse('http://127.0.0.1:8000/process-input/');
      ...
      if (response.statusCode == 200) {
        ...
        // Assuming responseData contains a key "next_sentence" with the new sentence.
        setState(() {
          // Replace the current sentence with the new one.
          // Ensure this logic aligns with how you're managing sentences in your app.
          sentences[currentSentenceIndex] = responseData['next_sentence'];
        });
        print('Next sentence updated: ${responseData['next_sentence']}');
      } else {
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                storeAttempts();
                print(
                    "Sending data: Total Attempts: $totalAttempts, Time Consumed: $currentTimeConsumed, Attempts per Punctuation: $attemptsPerPunctuation");
                sendDataToBackend();
                nextSentence();
                setState(() {
            ...
            //TextButton(onPressed: sendDataToBackend, child: Text('test')),
          ],
        ),
      ),
    );
